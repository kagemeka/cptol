...
def main() -> None:
    ...
    values = a + b + c + d
    compress = ArrayCompression(values)
    a = [compress(x) for x in a]
    b = [compress(x) for x in b]
    c = [compress(x) for x in c]
    d = [compress(x) for x in d]

    mx = max(max(a), max(b), max(c), max(d))
    s = FenwickTree(mx + 1)
    # heap
    # manage y in heap
