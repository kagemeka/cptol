...
class AtCoder:
    ...
    class ABC060:
        ...
        def d():
            ...
            @lru_cache(maxsize=None)
            def dfs(i, j, k):
                if i >= len(v[0]) or j >= len(v[1]) or k >= len(v[2]):
                    return
                w = j + 2 * k + (i + j + k) * w0
                if w > W:
                    return
                ...
                global res
                res = max(res, v[0][i] + v[1][j] + v[2][k] + v[3][l])
                dfs(i + 1, j, k)
                dfs(i, j + 1, k)
                dfs(i, j, k + 1)
    ...
    class ABC067:
        ...
        def a():
            a, b = map(int, sys.stdin.readline().split())
            print(
                "Impossible" if a % 3 and b % 3 and (a + b) % 3 else "Possible"
            )
    ...
    class ABC068:
        ...
        def b():
            print(
                2 ** math.floor(math.log2(int(sys.stdin.readline().rstrip())))
            )
    ...
    class ABC082:
        ...
        def d():
            s = [
                1 if c == "T" else 0 for c in sys.stdin.readline().rstrip()
            ] + [1]
            x, y = map(int, sys.stdin.readline().split())
            i = j = 0
            while s[i] == 0:
            ...
            print(
                "Yes"
                if possible(d[0], abs(x)) & possible(d[1], abs(y))
                else "No"
            )
    ...
    class ABC085:
        ...
        def d():
            ...
            np.cumsum(b, out=b)
            print(
                np.searchsorted(b, h, side="left") + 1
                if h <= b[-1]
                else len(b) + (h - b[-1] + a - 1) // a
            )
    ...
    class ABC087:
        ...
        def d():
            ...
            for l, r, d in zip(*[iter(lrd)] * 3):
                l -= 1
                r -= 1
                g.add_edge(l, r, weight=d)
                g.add_edge(r, l, weight=-d)
    ...
    class ABC089:
        ...
        def d():
            ...
            yx = np.pad(
                np.argwhere(a)[np.argsort(a.ravel())],
                pad_width=[(0, d), (0, 0)],
                constant_values=0,
            )
            a = np.zeros(h * w + d, dtype=np.int32)
            for i in range(0, h * w - d, d):
                a[i + d : i + 2 * d] = a[i : i + d] + np.absolute(
                    yx[i + d : i + 2 * d] - yx[i : i + d]
                ).sum(axis=1)
            print(*(a[r] - a[l]), sep="\n")
    ...
    class ABC091:
        ...
        def d():
            ...
            c = 2 ** np.arange(30)
            a, b = np.sort(
                np.array(ab).reshape(2, n)[:, None] % (2 * c)[:, None]
            )
            res = 0
            for i in range(30):
                j = np.searchsorted(
                    b[i], np.arange(1, 5)[:, None] * c[i] - a[i]
                ).sum(axis=1)
                j[1::2] *= -1
                res += (j.sum() & 1) * c[i]
            print(res)
    ...
    class ABC093:
        ...
        def d():
            ...
            res += (a - 1) * (b - a <= 1)
            res += (
                x + np.minimum(x - a - 1 * (x * (x + 1) >= a * b), b - x - 1)
            ) * (b - a >= 2)
    ...
    class ABC100:
        ...
        def d():
            ...
            print(
                np.sort((op[:, None] * xyz).sum(axis=-1), axis=-1)[:, n - m :]
                .sum(axis=-1)
                .max()
            )
    ...
    class ABC101:
        ...
        def d():
            ...
            def f(n):
                return sorted(
                    [
                        pow(10, d) * (n // pow(10, d) + 2) - 1
                        for d in range(int(math.log10(n)) + 2)
                    ],
                    key=lambda x: x / s(x),
                )[0]
    ...
    class ABC102:
        ...
        def d_2():  # binary_search (offline)
            ...
            (p, q), (s, r) = f(a), f(a[::-1])
            a = np.sort(
                np.vstack((p[:-1], q[:-1], r[-2::-1], s[-2::-1])), axis=0
            )[:, 1:-1]
            print((a[-1] - a[0]).min())
    ...
    class ABC114:
        ...
        def d():
            ...
            if n == 1:
                ...
                return
            c = np.bincount(
                np.array([*pn.factorize_factorial(n).values()]), minlength=75
            )
    ...
    class ABC115:
        ...
        def d():
            n, x = map(int, sys.stdin.readline().split())
            l, p = [None] * (n + 1), [None] * (n + 1)
            l[0] = p[0] = 1
            for i in range(n):
                l[i + 1], p[i + 1] = 2 * l[i] + 3, 2 * p[i] + 1
            ...
            def f(i, x):
                if i == 0:
                    ...
                    return p[i - 1] + 1 + f(i - 1, x - 1 - l[i] // 2)
    ...
    class ABC119:
        ...
        def d():
            ...
            s, t, x = (
                np.array([-inf] + I[:a] + [inf]),
                np.array([-inf] + I[a : a + b] + [inf]),
                np.array(I[-q:]),
            )
    ...
    class ABC123:
        ...
        def d_1():
            ...
            def f(a, b):
                return np.sort((a[:, None] + b).ravel())[-1 : -1 - k : -1]
        ...
        def d_2():
            x, y, z, K, *I = map(int, sys.stdin.read().split())
            a, b, c = (
                sorted(I[:x], reverse=1),
                sorted(I[x : x + y], reverse=1),
                sorted(I[x + y :], reverse=1),
            )
            added = set()
            q = [(-(a[0] + b[0] + c[0]), 0, 0, 0)]
            added.add((0, 0, 0))
            ...
            for _ in range(K):
                ...
                if i < x - 1 and not (i + 1, j, k) in added:
                    heappush(q, (-(v - a[i] + a[i + 1]), i + 1, j, k))
                    added.add((i + 1, j, k))
                if j < y - 1 and not (i, j + 1, k) in added:
                    heappush(q, (-(v - b[j] + b[j + 1]), i, j + 1, k))
                    added.add((i, j + 1, k))
                if k < z - 1 and not (i, j, k + 1) in added:
                    heappush(q, (-(v - c[k] + c[k + 1]), i, j, k + 1))
                    added.add((i, j, k + 1))
            print(*res, sep="\n")
    ...
    class ABC181:
        ...
        def d():
            ...
            if len(n) <= 2:
                print(
                    "Yes" if int(n) % 8 == 0 or int(n[::-1]) % 8 == 0 else "No"
                )
                return
            c = Counter(n)
        ...
        def e():
            ...
            r = tmp.copy()
            r[1::2] = 0
            np.cumsum(r[::-1], out=r[::-1])
            i = np.searchsorted(w, h)
            d = np.pad(h[2:] - h[:-2], 1, constant_values=0)
            d[::2] = 0
            d += np.minimum(
                np.abs(h - w[np.maximum(i - 1, 0)]),
                np.abs(h - w[np.minimum(m - 1, i)]),
            )
            print((l[:-1] + r[1:] + d).min())
        ...
        def f():
            ...
            y = xy[:, 1]
            if n == 1:
                print(np.maximum(100 - y, y + 100)[0] / 2)
                return
            ...
            def f(r):
                r *= 2
                uf = GeometryTopology.Graph(n + 2)
                uf.init_dsu()
                for i in np.argwhere(y + 100 <= r).ravel():
                    uf.unite(i, n)
                for i in np.argwhere(100 - y <= r).ravel():
                    uf.unite(i, n + 1)
                for i, j in ij[np.argwhere(d <= r * r).ravel()]:
                    uf.unite(i, j)
    ...
    class ABC182:
        ...
        def c():
            ...
            i = np.arange(3)
            for d in n:
                dp[i] = np.maximum(dp[i], dp[(i - d) % 3] + 1)
    ...
    class ARC107:
        ...
        def d():
            ...
            def solve(n, k):
                dp = np.zeros((n + 1, 2 * n + 1), dtype=np.int64)
                dp[0, 0] = 1
                for i in range(1, n + 1):
    ...
    class ALPC:  # AtCoder Library Practice Contest\
        ...
        def g():
            n, m, *ab = map(int, sys.stdin.read().split())
            a, b = np.array(ab).reshape(m, 2).T
            _, r = connected_components(
                csr_matrix(([1] * m, (a, b)), (n, n)), connection="strong"
            )
            groups = [[] for _ in range(n)]
            for u in range(n):
                groups[r[u]].append(u)
